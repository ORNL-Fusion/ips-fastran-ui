...
def get_nbi(shot, times, dtavg=20, ncfile=None):
    ...
    print(f'times = {times}, dtavg = {dtavg}')
    for k, src in enumerate(nb_list):
        src0 = src.split('_')[0]
        oanb = len(src.split('_')) == 2
        print(src0, oanb)
        for i in range(num_times):
            ind = (time_mds >= times[i] - 0.5 * dtavg) & (time_mds <= times[i] + 0.5 * dtavg)
            ...
            if oanb:
                ...
                scale = 1.0
            pnbi[k, i] *= scale
            if pnbi[k, i] <= 3.5e4 * scale:
                pnbi[k, i] = 0.0
        einj[k] = einj_mds[src0]
        nbsrc[k] = src
        mix = d3dbeam.beam_species_mix(max(einj[k] * 1.e-3, 20.0))
        ffull[k] = mix['after'][0]
        fhalf[k] = mix['after'][1]

    # for k in range(len(nb_list)):
    #   print(f'{einj[k]}, {ffull[k]}, {fhalf[k]}')
