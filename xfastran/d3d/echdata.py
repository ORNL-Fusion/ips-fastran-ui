...
def get_ech(shot, times, dtavg=20, ncfile='', outdir='.'):
    ...
    for k in range(num_systems):
        ...
        for i in range(num_times):
            ind = (ectime >= times[i] - 0.5 * dtavg) & (ectime <= times[i] + 0.5 * dtavg)
            pech[k, i] = average(pech_a[ind])
            if pech[k, i] <= 3.5e4 or isnan(pech[k, i]):
                pech[k, i] = 0.
    ...
    dim_time = nc.createDimension('time_ech', output['num_times'])

    ech = nc.createVariable('ech', 'i4', ('ech',))
    time = nc.createVariable('time_ech', 'f8', ('time_ech',))
    ...
    aziang = nc.createVariable('aziang', 'f8', ('ech', 'time_ech',))
    polang = nc.createVariable('polang', 'f8', ('ech', 'time_ech',))
    pech = nc.createVariable('pech', 'f8', ('ech', 'time_ech',))
    ...
    phai = zeros(num_systems)
    for k in range(num_systems):
        rfpow[k] = output['pech'][k][0]
        phai[k] = output['aziang'][k][0]
        thet[k] = output['polang'][k][0]
    ...
    intoray['adjust']['width'] = num_systems * [-1.]
    intoray['adjust']['width2'] = num_systems * [-1.]
    intoray['adjust']['j_multi'] = num_systems * [1.]
 
    intoray['edata']['igafit'] = [1]
    intoray['edata']['netcdfdat'] = [True]

    f_intoray = os.path.join(outdir, 'intoray_%06d' % shot)
    print('writing', f_intoray)
    intoray.write(f_intoray)
