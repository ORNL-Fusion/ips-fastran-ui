...
def wrt_netcdf(ishot, efitdir, efit_id='', tmin=0, tmax=sys.maxsize, nbdry_max=101, ncfile='', bin_fluxsurf=''):
    ...
    li = np.zeros(ntimes)
    betan = np.zeros(ntimes)
    for k in range(ntimes):
        afile = os.path.join(efitdir, 'a%06d.%05d%s' % (ishot, times[k], efit_id))
...
#   var_nlim = nc.createVariable('nlim', 'i4', ('one',))
    ...
    var_rlim = nc.createVariable('rlim', 'f8', ('lim'))
    var_zlim = nc.createVariable('zlim', 'f8', ('lim'))
...
#   var_nlim[:] = nlim
    ...
    var_li = nc.createVariable('li', 'f8', ('time_eq',))
    var_betan = nc.createVariable('betan', 'f8', ('time_eq',))
    var_li[:] = li
    var_betan[:] = betan
